...
def parse_yolo_region(blob, resized_image_shape, original_im_shape, params, threshold, yolo_type):
    ...
    ), f"Invalid size of output blob. It sould be in NCHW layout and height should be equal to width. Current height = {out_blob_h}, current width = {out_blob_w}"
...
def main():
    ...
    while cap.isOpened():
        ...
        render_ori_message = (
            f"Image Origin size: {origin_im_size[1]}x{origin_im_size[0]}"
        )
        render_infer_message = "Image Inference size: 640x640"
        async_mode_message = (
            f"Async mode is on. Processing request {cur_request_id}"
            if is_async_mode
            else f"Async mode is off. Processing request {cur_request_id}"
        )
        ...
        if not args.no_show:
            ...
            if key == 9:
                ...
                log.info(f'Switched to {"async" if is_async_mode else "sync"} mode')
